...
function SidebarContent({ theme, glowColor, pathname, onNavigate, onOpenCalendar }: {
  theme: keyof typeof SIDEBAR; glowColor: string; pathname: string; onNavigate?: () => void; onOpenCalendar: () => void;
}) {
  ...
  return (
    <>
      ...
      {/*
        ...
        Вынесена ЗА пределы прокручиваемого блока выше и закреплена как
        shrink-0 футер сайдбара — теперь видна всегда, без прокрутки,
        независимо от того, насколько длинная страница открыта в контенте.
      */}
      <button onClick={() => { router.push("/"); onNavigate?.(); }}
        className={`mt-4 shrink-0 w-full py-2.5 text-xs font-black uppercase tracking-widest rounded-2xl transition-all ${NAV_FONT[theme]} ${theme === "classic" ? "bg-red-950/30 border border-red-900/40 text-red-400 hover:bg-red-900/40" : theme === "aurora" ? "bg-red-50 border border-red-200 text-red-500 hover:bg-red-100" : "border border-red-900/60 text-red-500/70 hover:bg-red-950/20"}`}>
        {copy.navQuit}
      </button>
    </>
  );
  ...
  return (
    <div className={`min-h-screen flex relative overflow-hidden ${theme === "aurora" ? "bg-[#fef6ff]" : "bg-[#03040a]"}`}>
      ...
      <aside className={`hidden lg:flex w-[240px] h-screen sticky top-0 flex-col p-5 shrink-0 relative z-20 overflow-hidden ${SIDEBAR[theme]}`}>
        ...
        <SidebarContent theme={theme} glowColor={glowColor} pathname={pathname} onOpenCalendar={() => setCalendarOpen(true)} />
      </aside>
      ...
      {mobileOpen && (
        <>
          ...
          <aside className={`lg:hidden fixed top-0 left-0 bottom-0 w-[280px] z-[80] flex flex-col p-5 overflow-hidden ${SIDEBAR[theme]}`}
            ...
            <div className="mt-8 flex-1 flex flex-col min-h-0 overflow-hidden">
              <SidebarContent theme={theme} glowColor={glowColor} pathname={pathname} onNavigate={() => setMobileOpen(false)} onOpenCalendar={() => setCalendarOpen(true)} />
            </div>
          </aside>
        </>
      )}
