...
export async function GET(req: Request) {
  ...
  if (hasNarrowingFilter) {
    ...
    // размер вменяемый (сотни, не тысячи), урезать по рейтингу не нужно.
    // Сортируем по рейтингу для удобства просмотра, но БЕЗ отбрасывания
    // хвоста списка.
    players = players.sort((a, b) => (b.overall ?? 0) - (a.overall ?? 0)).slice(0, 300);
  } else {
