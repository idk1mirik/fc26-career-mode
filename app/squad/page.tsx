...
// ─── FORMATIONS ───────────────────────────────────────────────────────────────
// Координаты формаций перенесены в lib/formations.ts — теперь это общий
// источник правды и для этой вкладки, и для раскладки на поле в отчёте о
// матче (components/MatchPitch.tsx), чтобы расстановка выглядела одинаково
// в обоих местах.
...
export default function SquadPage() {
  ...
  const filteredPlayers = useMemo(() => players
    .filter(p => normalizeName(p.name).includes(normalizeName(search)))
