    ...
    <div className="overflow-x-auto pb-2">
      <div className="flex gap-4 min-w-max px-1">
        {stages.map(stage => {
          ...
          return (
            <div key={stage.name} className="flex flex-col gap-2 w-[220px] shrink-0">
              <div className={`text-[10px] font-bold uppercase tracking-widest font-display ${styles.stageLabel} mb-1`}>
                {stage.name}
              </div>
              ...
              {singles.map(f => {
                ...
                if (f.is_bye) {
                  return (
                    <div key={f.id} className={`rounded-xl p-2.5 ${styles.card} ${isUser ? "ring-1 ring-emerald-400/50" : ""}`}>
                ...
                return (
                  <div key={f.id} className={`rounded-xl p-2.5 card-lift animate-fade-in-up ${styles.card} ${isUser ? "ring-1 ring-emerald-400/50" : ""}`}>
